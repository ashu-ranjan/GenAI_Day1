...
async def call_openai(problem: str) -> dict:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            log.info("OpenAI request attempt %d", attempt)
            ...
            log.warning("OpenAI HTTP error %d", e.response.status_code)
            if e.response.status_code == 401:
                raise HTTPException(status_code=401, detail="Invalid OPENAI_API_KEY.")
            last_error = HTTPException(status_code=502, detail=f"OpenAI API error: {e.response.status_code}")
        except httpx.TimeoutException:
            log.warning("OpenAI timeout on attempt %d", attempt)
            last_error = HTTPException(status_code=504, detail="OpenAI API timed out.")
...
async def solve(req: SolveRequest):
    ...
    result = await call_openai(req.problem)
